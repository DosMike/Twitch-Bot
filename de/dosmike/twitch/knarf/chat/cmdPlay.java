...
import de.dosmike.twitch.knarf.ClientStorage;
	...
	public void run(String user, ChatRank rank, String[] args, boolean silent) {
		...
		if (args[0].equalsIgnoreCase("list")) {
			...
			if (l.isEmpty())
				...
				getTwitch().sendChat("These are all sounds:" + l.substring(1));
			return;
		}
		...
		if (Executable.cfg.hasKey("Sounds", args[0])) {
			String[] sounds = Executable.cfg.get(args[0]).split(";");
			playSound(sounds[rng.nextInt(sounds.length)]);
		} else {
